...
export function loadModule(moduleLoaderFunction: ModuleLoaderFunction): Module {
  ...
  if (__DEBUG__) {
    console.log(
      `[dynamicRequireCache] loadModule("${moduleLoaderFunction.name}")`,
    );
  }
  ...
  if (!record) {
    ...
    moduleLoaderFunction().then(
      module => {
        if (__DEBUG__) {
          console.log(
            `[dynamicRequireCache] loadModule("${moduleLoaderFunction.name}") then()`,
          );
        }
    ...
    let timeoutID = setTimeout(function onTimeout() {
      if (__DEBUG__) {
        console.log(
          `[dynamicRequireCache] loadModule("${moduleLoaderFunction.name}") onTimeout()`,
        );
      }
